...
if(!function_exists('getFormatedDate')){
	function getFormatedDate($date, $only_date = false)
	{
...
if (!function_exists('rtl')) {
	function rtl(){
		if(\Illuminate\Support\Facades\Auth::check() and auth()->user()->language)
		{
			$language = Language::where('code', auth()->user()->language)->first();

			return (bool) $language->rtl;
		}else{
			return (bool) config('configs')->where('key', 'ttl_rtl')->first()->value;
		}
	}
}
...
if (!function_exists('getConfig')) {
    ...
    {
        $config = config('configs')->where('key', $key)->first();
        if ($config){
            return $config->value;
        }
        return $default;
...
if (!function_exists('isStorageLimitExceeded')) {
    ...
    function isStorageLimitExceeded()
    {
        ...
        if ($subscription){
            ...
            $root_uploads = getRootUploadsDirSize();
            $public_uploads = getPublicUploadsDirSize();
            $total_storage_size = 0;
            if($db_size > 0){
                $total_storage_size += $db_size;
            }
            if($root_uploads > 0){
                $total_storage_size += $root_uploads;
            }
            if($public_uploads > 0){
                $total_storage_size += $public_uploads;
            }
            if($subscription->disk_storage_limit <= $total_storage_size && auth()->user()->role_id > 1){
                $is_expired = true;
            }
        }
        return ['status' => $is_expired, 'message' => 'System storage limit is exceeded. Please contact the System Administrator.'];
    }
    function getSizeDiskStatus()
    {
        $used_total_storage_size = 0;
        $db_size = getDBSizeInMB();
        $root_uploads = getRootUploadsDirSize();
        $public_uploads = getPublicUploadsDirSize();
        if($db_size > 0){
            $used_total_storage_size += $db_size;
        }
        if($root_uploads > 0){
            $used_total_storage_size += $root_uploads;
        }
        if($public_uploads > 0){
        ...
        $balance_storage_size = $disk_storage_limit-$used_total_storage_size;
        return array("disk_storage_limit"=>$disk_storage_limit,"used_total_storage_size"=>$used_total_storage_size,"balance_storage_size"=>$balance_storage_size);
    }
}

if (!function_exists('getSystemSubscription')) {
...
if (!function_exists('getSystemSubscriptionAllowedLawyers')) {
    ...
    function getSystemSubscriptionAllowedLawyers()
    {
        ...
        if ($subscription){
            ...
            $currentTimestamp = strtotime($current_date);
            $expiryTimestamp = strtotime($expiry_date);
            $availableOfLawyers = $subscription->allowed_no_lawyers;

            $registredOfLawyers = Lawyer::where('created_at', '>=', $subscription->activate_date)->where('created_at', '<', $expiry_date)->count();
 			$availableOfLawyers-=$registredOfLawyers;

            if($currentTimestamp >= $expiryTimestamp){
                $availableOfLawyers = 0;
            }
        }
        return $availableOfLawyers;
    }
}
...
if (!function_exists('getUploadsDirSize')) {
    function getRootUploadsDirSize()
    {
        $file_size = 0;
        foreach( \File::allFiles(base_path('uploads')) as $file)
        {
...
if (!function_exists('getUploadsDirSize')) {
    function getPublicUploadsDirSize()
    {
        $file_size = 0;
...
if (!function_exists('getSubscriptionDays')) {
