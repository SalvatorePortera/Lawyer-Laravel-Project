...
@section('mainContent')
                                    ...
                                    <div class="primary_input mb-15">
                                        ...
                                        <select class="primary_select mb-15" name="month" id="month">
                                            @foreach ($months as $month)
                                                ...
                                                @else
                                                    <option value="{{ $month }}" {{$month == \Carbon\Carbon::now()->monthName ? 'selected' : ''}}>{{ __('common.'.$month) }}</option>
                                                @endisset
                                            @endforeach
                                        </select>
                                    ...
                                    <table class="table Crm_table_active3">
                                        <thead>
                                            <tr>
                                                ...
                                                <th scope="col">{{ __('attendance.Present') }}</th>@foreach ($report_dates as $key => $report_date) <th scope="col">{{ $report_date->date }}</th>@endforeach
                                            </tr>
                                        </thead>
                                        <tbody>
						...
						@php
							...
							$i=0;
                                                ...
                                                <tr>
                                                    ...
                                                    <td>{{ $absent }}</td>
                                                    <td>{{ $half_day }}</td>
                                                    <td>@if($user->attendances){{number_format($total_attendance, 2)}}% @else 00 @endif</td>
						    @if (sizeof($attendances) > 0 && sizeof($attendances) == $max_col)
                                                        @foreach ($user->attendances->where('month', $m)->where('year', $y) as $attendance)@php $i++; @endphp @if($i>$total_days_of_month)@continue @endif<td>{{$i}}{{ $attendance->attendance }}</td> @endforeach
                                                    @elseif (sizeof($attendances) > 0 && sizeof($attendances) < $max_col)
                                                        @foreach ($user->attendances->where('month', $m)->where('year', $y) as $attendance) <td>{{ $attendance->attendance }}</td> @endforeach
                                                        @for ($i=$max_col_1; $i < $max_col; $i++) <td></td>  @endfor
                                                    @else
                                                        @for ($i=0; $i < $max_diff; $i++) <td></td>  @endfor
                                                    @endif
                                                </tr>
                                            @endforeach
                                        </tbody>
    ...
    <div class="modal fade admin-query" id="reportPrintForm"></div>
@endsection
