...
class TransactionService
{
    ...
    private function formatRequest($request, $transaction = null): array
    {
        ...
        $formattedRequest = [
            ...
            'case_id' => gv($request, 'case_id'),
            'clientable_type' => gv($request, 'clientable_type'),
            'clientable_id' => gv($request, 'clientable_id'),
            'payment_method' => gv($request, 'payment_method', 'cash'),
            'amount' => gv($request, 'amount'),
            'transaction_date' => date('Y-m-d', strtotime(gv($request, 'transaction_date'))),
        ...
        if (gv($request, 'service_type')){
            $formattedRequest['servicetype_id'] = gv($request, 'service_type');
        }
    ...
    public function getPreRequisite($type, $come_from, $id = null)
    {
        ...
        $data['expense_types'] = $this->service->pluckAllByType($type, $come_from);
        ...
        $client_id = $data['model']->clientable_id;
        
        

        $data['cases'] = Cases::where(function($q) use ($client_id) {
